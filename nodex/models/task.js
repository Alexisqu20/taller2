...
TaskModel.deleteById = (id) => {
    ...
    if(pos >= 0){
        ...
        for (let index = 0; index < TaskModel.tasks.length; index++) {
            const task = TaskModel.tasks[index];
            if(task.id != id){
                return "Tarea numero "+id+" eliminda satifactoriamente"
            }
...
TaskModel.newadd= (id) => {
    let newtask = new TaskModel(`title`,`Description`,new Date(),'https://picsum.photos/200/300.jpg')
    ...
    return "tarea no a sido guardada"
}
